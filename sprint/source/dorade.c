              ...
              float flddat[MXFLD][MXGAT])
{
  ...
  dorptr->bytesbeam = dorptr->bytesbeam + len;
  /*printf("in read rdat adding the bytes  %d  %d\n",len,dorptr->bytesbeam);*/
  nlen = 0;
  for(index = 0; index < 8; index++){
     ...
     if(test_char >= 65 && test_char <= 90){
	     ...
	     /*              printf("found a swib\n");*/
	      ...
	      /*printf("reading in the rdat block\n");*/
           ...
           else if(array[0] == 'X' && array[1] == 'S' && array[2] == 'T' && array[3] == 'F'){
	     rval = read(fd,junk,4);
	     array[4] = junk[0];
	     array[5] = junk[1];
	     array[6] = junk[2];
	     ...
	     *block = COMM;
	     ...
	     if (rval <= 0) {
	       printf("error reading in the xstf block\n");
	       exit(0);
	     }    
             dorptr->bytesbeam = dorptr->bytesbeam + len;
	     found_descriptor = 1;
