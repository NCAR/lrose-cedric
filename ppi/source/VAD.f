...
C     NIN2   - TYPE OF OUTPUT ('FOUR', 'LSQR','RESD', 'MEASff.f')
...
c-----debug
     ...
     +        ' kvd,uv_vd=',kvd,U_VD(KVD),V_VD(KVD)
c-----debug
      IF(C1.GT.0.0)THEN
         CNTMN=C1
      ELSE
...
c-----debug
      print *,'mn-mxgate,mxg,mxr=',mngate,mxgate,mxg,mxr
...
c
c         This routine is either misnamed or missing
         CALL LABELFL(PLTSW)
c
         WRITE(99,13)MSCAN,AVTYP,ITIM1,ITIM2,
     X        NAMFLD(IIN1),IIN1,IFLD(IIN1),
     X        NAMFLD(IOUT),IOUT,IFLD(IOUT)
...
C
            IF(IIN2.GT.0 .AND. DAT(I,J,IIN2) .NE. BDVAL)THEN
...
C
               ...
               ye = y0
c-----debug
c               write(6,1700)NIN2(1:4),ri,ang,xi,yi,vri,gap,gapmx
...
C
            ...
            IF(NIN2.EQ.'FOUR')THEN
c-----debug
c               print *,'FOUR- Cnt,gapmx=',cnt,gapmx
c-----debug

C     NIN2='FOUR': Calculate wind parameters from Fourier
...
c-----debug
            ...
            ELSE IF(NIN2.EQ.'LSQR')THEN
c-----debug
c               print *,'LSQR- Cnt,gapmx=',cnt,gapmx
c-----debug

C     NIN2='LSQR': Calculate wind parameters from least-squares 
C                  fit and put these in /VADWINDS/ arrays
...
c
               ...
               shr(i,kvd)=shrl
c-----debug
c               print *,'LSQR- uvdthv=',ue,ve,divl,strl,shrl,vorl
c-----debug
